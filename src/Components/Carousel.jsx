            ...
            return <div key={index} className='bg-gradient-to-r from-[#0f0c29] via-[#302b63] to-[#24243e] -z-10'>
              <div className='flex flex-col md:flex-row gap-10 justify-center h-[600px] my-20 md:my-0 items-center px-4'>
                <div className='md:space-y-6 space-y-3'>
                  <h2 className='text-red-500 font-semibold font-sans text-sm'>Wear Your Confidence</h2>
                  ...
                  <button onClick={() => addToCart(item)} className='bg-gradient-to-r from-red-500 to-purple-500 text-white px-3 py-2 rounded-md cursor-pointer mt-2'>Buy Now</button>
                </div>
                <div>
                  <img src={item.image} alt={item.title} className='rounded-full w-[340px] hover:scale-105 transition-all shadow-2xl shadow-red-400' />
                </div>
              </div>
            </div>
          })
